...
```{r}
#| label: bioStrings
#| code-summary: Outputs visualization of file contents with the ``Biostrings::readDNAStringSet()`` package.
#| eval: false
...
r_file_path <- "01_Transform/data/magicicada.fasta"


print_dna <- function(x) {
  cat(silver("DNAStringSet object of length"), silver(length(x)), silver(":\n"))
  cat(silver("        width seq                                          names\n"))
  for (i in seq_along(x)) {
    seq <- as.character(x[[i]])
    colored_seq <- gsub("A", red("A"), seq)
    colored_seq <- gsub("C", green("C"), colored_seq)
    colored_seq <- gsub("G", yellow("G"), colored_seq)
    colored_seq <- gsub("T", blue("T"), colored_seq)
    cat(sprintf("[%d] %11d %s ... %s %s\n",
                i, width(x[i]), 
                substr(colored_seq, 1, 20),
                substr(colored_seq, nchar(seq) - 19, nchar(seq)),
                names(x)[i]))
  }
}
...
fasta_file = "01_Transform/data/magicicada.fasta"
